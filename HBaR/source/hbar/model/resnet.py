...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, rob=False, reg_feature_dim=128):
    ...
    def forward(self, x, mcrat=0):
        ...
        output_list.append(outL)
        
        #out = self.linear(out)
        out = F.log_softmax(self.linear(outL), dim=1)
        #output_list.append(out)
...
def ResNet18(**kwargs):
    ...
    return ResNet(BasicBlock, [2,2,2,2], num_classes=10, rob=rob)
...
def ResNet50(**kwargs):
    rob = kwargs['robustness'] if 'robustness' in kwargs else False
    return ResNet(Bottleneck, [3,4,6,3], num_classes=100, rob=rob)
