...
def get_hsic_epoch(config_dict, model, dataloader, n_classes):
    """ Computes the hsic
    """
    acc = []
    loss = []
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(dataloader):
            ...
            h_target = to_categorical(h_target, num_classes=n_classes).float()
...
def to_categorical(y, num_classes):
    ...
    return torch.squeeze(torch.eye(num_classes).to(y.device)[y])
