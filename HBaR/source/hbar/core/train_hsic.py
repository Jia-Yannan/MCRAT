...
def hsic_train(cepoch, model, data_loader, optimizer, scheduler, config_dict):
    criterion1 = MaximalCodingRateReduction(eps=config_dict['epsMCR2'])
    if config_dict['data_code'].upper() == 'MNIST' or config_dict['data_code'].upper() == 'CIFAR10':
        n_classes=10
    elif config_dict['data_code'].upper() == 'CIFAR100':
        n_classes=100
    ...
    for batch_idx, (data, target) in pbar:
        ...
        if config_dict['adv_train']:
            ...
            output, hiddens, outF = model(attacked_data, 1)
        ...
        h_target = misc.to_categorical(h_target, num_classes=n_classes).float()
        ...
        if config_dict['mcrAt']==1 and config_dict['adv_train']:
            loss1, _, _ = criterion1(outF, target, num_classes=n_classes)
            if config_dict['data_code'].upper() == 'MNIST':
                k = 0.02
            else:
                k = 0.005
            # print('k', k)
