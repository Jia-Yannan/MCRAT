...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0, reg_feature_dim=128):
        ...
        self.reshape = torch.nn.Sequential(
            nn.Linear(nChannels[3], 512, bias=True),
            nn.BatchNorm1d(512),
    ...
    def forward(self, x, mcrat=0):
        ...
        if mcrat==1:
            #--
            outF = self.reshape(out)
            outF = F.normalize(outF)
            #--
            return self.fc(out), outF
        else:
            return self.fc(out)
